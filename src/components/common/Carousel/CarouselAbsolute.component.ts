...
export default class Carousel extends Vue {
  ...
  mounted() {
    this._container = <HTMLElement>document.querySelector('#carousel-absolute-image-container');
    this._pagination = <HTMLElement>document.querySelector('#carousel-absolute-pagination');
  ...
  private async _executeTransition(index: number) {
    ...
    await this._requestAnimationFrame(() => {
      ...
      this._container.style.transform = 'translate(0, 0)';   // 恢复left
    });
  ...
  public pointerMoveHandler(ev: PointerEvent) {
    ...
    this._container.style.transform = `translate(${this._movement.x}px, 0)`;
  }
